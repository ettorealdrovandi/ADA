...
% Resolves OpenDyslexic via the system font search path.
% Requires the OpenDyslexic OTF files to be installed in a standard
% font location: ~/Library/Fonts/ on macOS, ~/.fonts/ or
% /usr/share/fonts/ on Linux, %WINDIR%\Fonts on Windows.
...
\usepackage{hyperref}
\hypersetup{
  pdftitle  = {Mathematical Accessibility Test (OpenDyslexic)},
...
\title{Mathematical Accessibility Test (OpenDyslexic)}
...
Complex analysis studies functions $f\colon U \to \symbb{C}$ that are differentiable
...
\begin{theorem}[Residue Theorem]
  Let $U \subset \symbb{C}$ be open and $f$ holomorphic on $U$ except at finitely many
...
In classical vector calculus, taking $\Omega$ to be a surface $S \subset \symbb{R}^{3}$
...
\begin{corollary}[Green's Theorem]
  ...
  $D \subset \symbb{R}^{2}$ and smooth functions $P$, $Q$,
  \begin{equation}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[>=Stealth, thick, font=\small,
    alt={Diagram of the complex plane. A circle of radius 2 centred at the origin
         is drawn counter-clockwise and labelled gamma. Two crosses mark the poles
         of the integrand at z equals 1 and z equals minus 1, both inside the contour.
         Tick marks label the real and imaginary axes from minus 2 to 2.}]
    % Axes
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=0.5\linewidth,
    alt={W3C PNG test image showing the word Testing with colour fades and
         alpha transparency, used here as a sample image.}]{alphatest}
