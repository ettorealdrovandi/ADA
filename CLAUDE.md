...
`tagging-real-world/build.sh` copies sources to a fresh output dir (originals untouched), applies 14 transformation phases (see *Defensive build pipeline* below; Phase 12 has three sub-steps 12a/12b/12c), then runs `latexmk -lualatex=lualatex-dev` and (opt-in) a light PDF/UA conformance check. CLI: `--ua=1|--ua=2` (default 2), `--only=N[,N…]`, `--skip=N[,N…]`, `--stop-after=N`, `--no-autofix`, `--validate`, `--dry-run`.
...
1. **Setup** — copy `*.tex`, `*.png`, `*.jpg`, `*.pdf`, `*.svg`, `*.PNG` into `tagged_output/`. Also removes any stale main PDF in the output dir so Phase 13's success check can't be fooled by a leftover from a previous run.
2. **Strip sub-file standalone preambles** — subfiles wrap their preamble + `\begin{document}` in `\begin{comment}…\end{comment}` (some are broken with `%\begin{comment}`); the script deletes everything from BOF through `\end{comment}` and removes any stray `\begin{document}` / `\end{document}` lines that may remain in subfiles.
...
6. **Legacy LaTeX fixes** — patches patterns that `pdflatex` tolerated but `lualatex-dev` + tagging do not. Sub-steps: (a) `$$…$$` → `\[…\]` via an inline-math-aware pair-toggle awk (preserves the `$X$$Y$` typo case; skips inside verbatim/code/picture-family envs — `verbatim`, `Verbatim`, `BVerbatim`, `LVerbatim`, `SaveVerbatim`, `lstlisting`, `minted`, `alltt`, `comment`, `forest`, `tikzpicture` — where `$$` may be a literal token rather than display math, e.g. forest's `[$$ ...]` empty-node placeholder); (b) `\\\\` inside `align`/`align*` collapsed to `\\`; (c) standalone `\hfill\\` / `\hfill\\\\` / bare `\hfill` lines deleted; (d) trailing `\\` at end of a line removed when the next line is blank or begins with `\item`/`\end{}`/`\begin{}`/sectioning commands; (e) trailing `\\` on `\item` lines removed; (e2) `\\` immediately after `\]` or `\)` removed; (f) standalone `\vfill\eject` → `\clearpage`; (g) manual `\setlength{\textwidth/textheight/topmargin/oddsidemargin/evensidemargin}` replaced with a single `\usepackage[paper=letterpaper,total={6in,8.5in}]{geometry}`; (h) inline math `$X$` inside section-level commands wrapped with `\texorpdfstring{$X$}{}` to keep `\mitOmega` and friends out of the `.aux`/TOC; (i) injects a `\tikzset{decoration/.code={\pgfkeys{/pgf/decoration={#1}}}}` shim after `\usepackage{tikz}` so the historical `decoration={markings,…}` shorthand keeps working in pgf 3.1.11a (TeX Live 2026), which now declares only `/pgf/decoration` and not `/tikz/decoration`; (j) corpus-conditional `pythonhighlight` replacement — when `\usepackage{pythonhighlight}` is present in any subfile, the package line is deleted and `\begin{python}…\end{python}` is rewritten to `\begin{verbatim}…\end{verbatim}` (built-in, tagpdf-aware). `pythonhighlight` builds on `listings`, which the [LaTeX3 tagging project](https://latex3.github.io/tagging-project/tagging-status/) lists as `currently-incompatible`; the `\begin{python}` env triggers a tagpdf `para hooks differ` Emergency stop. Syntax highlighting is lost.
...
12. **Normalize env line breaks + validate `\begin`/`\end` balance + detect prose-on-prose nesting** — three sub-steps. **12a (normalize)**: rewrites each `.tex` so every `\begin{X}` and `\end{X}` sits on its own line; mid-line whitespace remnants that would become a spurious `\par` are suppressed; verbatim-/picture-family env bodies are passed through unchanged; idempotent. The canonical layout makes 12b's diagnostics easier to read and is a prerequisite for 12c's line-based splice. **12b (validate)**: stack-based per-file scan that auto-fixes the two safe cases (unclosed env at EOF → append `\end{X}`; orphan `\end{X}` alone on a line → delete that line) and hard-fails on the two unsafe cases (inline-content orphan; genuine mismatched nesting). Skips inside the same verbatim-/picture-family envs as Phase 6a, but correctly parses `\begin{X}` commands that appear *before* a verbatim begin on the same line (e.g. `\begin{center}\begin{tikzpicture}`). **12c (prose-on-prose nesting)**: detects a prose-family env nested inside another `\newtheorem`-derived env — a proven trigger of the tagpdf "automatic begin and end text-unit para hooks differ" error. Detection-only set comprises every env declared via `\newtheorem{X}` / `\newtheorem*{X}` in the corpus, plus `proof` (amsthm). The outer-ancestor lookup uses only `\newtheorem`-derived envs (proof is leaf-ish, no counter). The "any prose ancestor on stack" rule catches `example > tcolorbox > remark`-style sandwiches. Two autofix strategies, picked by inner env type. **UNWRAP** (inner = `\newtheorem`-derived) deletes the inner's `\begin{...}` and matching `\end{...}` lines so the content survives as plain prose; outer numbering preserved, inner semantic marker lost. **SPLIT** (inner = `proof`, with the prose ancestor as its *immediate* parent) closes the outer before the inner `\begin{proof}`, hoists the proof to top scope, and either reopens the outer after `\end{proof}` or elides the original `\end{OUTER}` (elide mode — chosen when the lines between `\end{proof}` and the original `\end{OUTER}` are only whitespace, %-comments, or spacers like `\vspace`/`\medskip`/`\noindent`/`\par`). Preserves the `Proof.…∎` structure element. Proof-in-prose cases with **intermediate envs** (e.g. `example > itemize > proof`) are reported as **SKIP** — splicing through intermediates would corrupt the structure, and these compile cleanly in practice. Loops scan-and-fix (max 50 iters); UNWRAPs apply in batch per iteration (line-disjoint deletes are safe); at most one SPLIT applies per iteration (split splices may overlap when multiple proofs share an outer). `--no-autofix` suppresses all three sub-steps and hard-fails on any defect; SKIP records warn but do not fail.
13. **Compile** — `latexmk -lualatex -lualatex=lualatex-dev -synctex=1`. `lualatex-dev` is left in interactive mode so real errors surface during the compile phase instead of being silently skipped past. On any compile failure (latexmk non-zero exit or missing PDF), the phase prints a multi-line warning naming representative partly-compatible packages (`amsthm`, `tcolorbox`, `fancyvrb`) and recommending a re-run; the warning surfaces a specific breadcrumb when the tagpdf "para hooks differ" signature is present in the log. A partial PDF (if produced) is retained for partial-credit acceptance by downstream tools.
14. **PDF/UA validation (opt-in)** — light conformance check: reads PDF metadata via `pdfinfo` (if installed) and optionally runs `verapdf --flavour ua1` or `--flavour ua2` (matching the build flavour, if installed). Neither tool is a hard dependency. **Off by default** — pass `--validate` (or `--only=14`) to run it.

The script ships with `--only=N[,N…]`, `--skip=N[,N…]`, `--stop-after=N`, `--no-autofix`, `--validate`, and `--dry-run` flags for debugging which phase introduced a problem. Phases are idempotent: re-running on a partially-built output directory is safe. When a real-world source fails to build accessibly, fix it by adding/adjusting a phase in `tagging-real-world/build.sh` rather than editing the original `.tex`.
